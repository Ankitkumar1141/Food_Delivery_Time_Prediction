...
# Setup data cleaning module logger
logger = logging.getLogger("data_cleaning")
logger.setLevel(logging.INFO)

# Configure console logging stream
...
# Define timestamped message format
...
def data_cleaning(data: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ...
        .assign(
            ...
            # Cast rider age attribute to float numeric type
            age = lambda x: x['age'].astype(float),
            # Cast driver rating metric to float numeric type
            ratings = lambda x: x['ratings'].astype(float),
            # Rectify coordinate polarity by enforcing non-negative magnitudes
            ...
            # Parse calendar timestamps and generate temporal features
            ...
            # Cast delivery batch size count to numeric float
            multiple_deliveries = lambda x: x['multiple_deliveries'].astype(float),
            # Sanitize target delivery duration in minutes
...
# Derive calendar date attributes: day, day of week, month, and year
...
def perform_data_cleaning(data: pd.DataFrame, saved_data_path: Path) -> None:
    ...
    # Write sanitized dataset to target storage location
