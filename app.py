...
# Configure scikit-learn transformer output to pandas DataFrame
set_config(transform_output='pandas')

# Authenticate and initialize DagsHub repository tracking
import dagshub
...
# Numerical and categorical feature groupings for data preprocessing
num_cols = ["age",
            "ratings",
            "pickup_time_minutes",
            "distance"]
...
# Initialize MLflow client interface
client = MlflowClient()

# Retrieve target model metadata from saved run artifacts
model_name = load_model_information("run_information.json")['model_name']

# Specify target deployment lifecycle stage
stage = "Production"

# Construct MLflow registry model path
model_path = f"models:/{model_name}/{stage}"

# Fetch production model from MLflow registry
model = mlflow.sklearn.load_model(model_path)

# Path to fitted data preprocessor
preprocessor_path = "models/preprocessor.joblib"
preprocessor = load_transformer(preprocessor_path)

# Assemble unified preprocessing and regressor inference pipeline
...
# Inference endpoint for delivery time estimation
...
def do_predictions(data: Data):
    ...
    # Execute raw data sanitization and feature extraction
    cleaned_data = perform_data_cleaning(pred_data)
    # Generate predicted delivery duration in minutes
